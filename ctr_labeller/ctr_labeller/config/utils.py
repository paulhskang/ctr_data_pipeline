...
def autoconfig(*classes_and_callables: Type, exclude: List[Callable] = None) -> Any:
    ...
    """
    import ctr_labeller.config._hydrazen as hydrazen_parser
    import ctr_labeller.config._tyro as tyro_parser
...
def instantiate(config, **kwargs):
    ...
    import _hydrazen as hydrazen_parser
...
def parse_config(config_type, yaml_arg: Optional[str] = '--config'):
    ...
    """
    from ctr_labeller.config._tyro import parse_args_tyro
...
def write_config_to_yaml(config, path):
    ...
    """
    from ctr_labeller.config._tyro import write_config_to_yaml
